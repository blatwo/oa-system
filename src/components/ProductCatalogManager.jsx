...
import { getCatalog, createCatalogItem, updateCatalogItem, deleteCatalogItem } from '../db/products';

const LEVEL_LABELS = ['', '大类', '系列', '版本'];
const LEVEL_BG = { 1: 'primary.main', 2: '#1565c0', 3: '#ef6c00' };
...
export default function ProductCatalogManager() {
  ...
  const load = async () => {
    try { setItems(await getCatalog()); } catch (e) { /* */ }
  };
  useEffect(() => { load(); }, []);
  ...
  const handleSave = async () => {
    try {
      if (editing) {
        await updateCatalogItem(editing.id, form.name, 0, form.description);
        setMsg({ text: '已更新', severity: 'success' });
      } else {
        await createCatalogItem(form.parent_id, form.level, form.name, form.description);
    ...
    <Box sx={{ maxWidth: 860, mx: 'auto' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        <Typography variant="h5" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <CategoryIcon color="primary" /> 产品目录
        </Typography>
      ...
      <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Typography>
            ...
            {deleteTarget?.level === 1 && ' 将同时删除其下所有系列和版本！'}
            {deleteTarget?.level === 2 && ' 将同时删除其下所有版本！'}
          </Typography>
        </DialogContent>
        <DialogActions>
        ...
        <Paper key={l1.id} variant="outlined" sx={{ mb: 2.5, overflow: 'hidden', borderRadius: 2 }}>
          ...
          {l1.children.map(l2 => (
            <Box key={l2.id}>
              ...
              {/* Level 3 — table-style compact list */}
                ...
                <Box sx={{ ml: 6, pl: 2, pr: 2, py: 0.5 }}>
                  {l2.children.map(l3 => (
                    <Box key={l3.id} sx={{
                      ...
                      <Typography variant="body2" sx={{ fontWeight: 500, minWidth: 80, color: LEVEL_BG[3] }}>
                        {l3.name}
                      </Typography>
                      {l3.description && (
